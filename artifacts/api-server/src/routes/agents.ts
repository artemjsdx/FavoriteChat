...
router.post("/agents/validate-token", async (req, res): Promise<void> => {
  ...
  try {
    ...
    if (!data.ok) {
      res.json({ valid: false, error: "Invalid token", botId: null, username: null, firstName: null });
      return;
    }
    res.json({
      valid: true,
...
router.post("/agents/validate-api-key", async (req, res): Promise<void> => {
  ...
  try {
    if (apiType === "openrouter") {
      ...
      if (!r.ok) {
        res.json({ valid: false, error: "Invalid key", defaultModel: null, contextKb: null });
        return;
      }
      res.json({ valid: true, defaultModel: "google/gemma-3-27b-it:free", contextKb: null, error: null });
      return;
    }
    if (apiType === "favorite") {
      const base = apiUrl ?? "";
      if (!base) {
        res.json({ valid: false, error: "API URL required for FavoriteAPI", defaultModel: null, contextKb: null });
        return;
      }
      const r = await fetch(`${base}/api/v1/me`, { headers: { Authorization: `Bearer ${apiKey}` } });
      if (!r.ok) {
        res.json({ valid: false, error: "Invalid key or URL", defaultModel: null, contextKb: null });
        return;
      }
      const d = (await r.json()) as { key?: { default_model?: string; context_kb?: number } };
      res.json({
        valid: true,
        defaultModel: d.key?.default_model ?? "gemini-3.0-flash-thinking",
        contextKb: d.key?.context_kb ?? null,
        error: null,
      });
      return;
    }
    res.json({ valid: false, error: "Unknown API type. Supported: openrouter, favorite", defaultModel: null, contextKb: null });
  } catch (err) {
...
router.post("/agents", async (req, res): Promise<void> => {
  ...
  try {
    ...
    if (!pollingManager.isEnabled()) {
      const publicUrl = process.env["PUBLIC_URL"] ?? process.env["REPLIT_DOMAINS"]?.split(",")[0] ?? "";
      if (publicUrl) {
        const webhookUrl = `${publicUrl.startsWith("http") ? publicUrl : `https://${publicUrl}`}/api/webhook/${body.botToken}`;
        try {
          const wh = await fetch(`https://api.telegram.org/bot${body.botToken}/setWebhook`, {
            method: "POST",
          ...
          if (whData.ok) {
            ...
            agent.webhookRegistered = true;
          }
        } catch (e) {
          req.log.warn({ e }, "setWebhook failed (non-fatal)");
        }
      }
    }

    // Polling mode: start polling for this agent immediately
    if (pollingManager.isEnabled()) {
      void pollingManager.startAgent(agent);
    }
...
router.put("/agents/:id", async (req, res): Promise<void> => {
  ...
  // Sync polling state when active flag changes
  if (pollingManager.isEnabled() && b.active != null) {
    if (b.active) {
      void pollingManager.startAgent(agent);
    } else {
      void pollingManager.stopAgent(agent.id);
    }
  }
...
router.delete("/agents/:id", async (req, res): Promise<void> => {
  ...
  if (pollingManager.isEnabled()) {
    await pollingManager.stopAgent(id);
  }
...
function agentToJson(a: typeof agentsTable.$inferSelect) {
  return {
    ...
    minDelaySec: a.minDelaySec,
    maxDelaySec: a.maxDelaySec,
    active: a.active,
